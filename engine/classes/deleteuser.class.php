<?php
    class DeleteUser extends CoreAdmin {
        ...
        public function getTitle() {//клас наслідує core відповідно перевизначає gettitle
            ...
            return tr::gettranslation(87, $lang);//повертаємо заголовок сторінки
        }
        ...
        public function getContent() {//getcontent функція
                ...
                if(isset($_POST['delete-user'])) {//якщо в запиті було видалення 
                    if(isset($_POST['list-delete'])) {//отримуєтсья списко на видалення
                        ...
                        for($i = 0; $i < count($list); $i++) {//перебір масиву
                            ...
                            if(1) {
                                $mysqli->query("DELETE FROM users WHERE id='%item1%'", $this->toInteger($list[$i]));
                ...
                //далі формується список користувачів //і виводитсья цей  список як форма для видалення
                if($result && $result->num_rows > 0) {
                    $row = $mysqli->assoc($result);
                    if($row) {
                        $result_content .= "<form method='post'>";
                        $result_content .= "<h1 class='h2'>".tr::gettranslation(87, $lang)."</h1><br>";
                        
                        do
                        {
                        $result_content .= "<p class='news-edit'><input type='checkbox' value='{$row['id']}' name='list-delete[]' /><span style='margin-left: 10px;'>[ {$row['login']} ] - {$row['full_name']}</span></p>";
                        }
                        while($row = $mysqli->assoc($result));

                        $result_content .= "<p class='form-element'><input class='btn btn-primary' type='submit' id='reg-user-btn' name='delete-user' value='".tr::gettranslation(68, $lang)."' /></p>
                        <div class='line'></div>";
